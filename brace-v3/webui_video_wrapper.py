...
new_switch = (
    ...
    "if(bgVideo){bgVideo.addEventListener('canplay',function(){document.body.classList.add('video-ready')});bgVideo.addEventListener('error',function(){document.body.classList.remove('video-ready')});}"
    "function setActiveMedia(el){var ts=document.querySelectorAll('.photo-thumb,.media-chip');for(var i=0;i<ts.length;i++)ts[i].classList.remove('active');if(el)el.classList.add('active')}"
    "function switchPhoto(f,el){if(bgPhoto)bgPhoto.src='/assets/'+f;document.body.classList.remove('video-ready');if(bgVideo){try{bgVideo.pause()}catch(_e){}}setActiveMedia(el)}"
    "function switchVideo(f,el){if(bgVideoSource){bgVideoSource.src='/assets/'+f}if(bgVideo){bgVideo.load();var p=bgVideo.play();if(p&&p.catch){p.catch(function(){document.body.classList.remove('video-ready')})}}setActiveMedia(el)}"
)
