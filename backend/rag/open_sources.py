    ...
    def fetch_works(
        ...
        for work in data["results"]:
            ...
            meta = Level1_Metadata(
                doc_id=hashlib.md5(str(work.get("id", "")).encode()).hexdigest(, usedforsecurity=False)[:16],
...
class CrossrefConnector:
    ...
    def fetch_works(
        ...
        for item in data["message"].get("items", []):
            ...
            meta = Level1_Metadata(
                doc_id=hashlib.md5(item.get("DOI", "").encode()).hexdigest(, usedforsecurity=False)[:16],
                titolo=titolo[:200],
                autore=autore[:100],
                anno=anno,
                lingua="en",
...
class WikipediaConnector:
    ...
    def search_articles(self, query: str, limit: int = 20) -> list[Level1_Metadata]:
        ...
        for item in data["query"].get("search", []):
            ...
            meta = Level1_Metadata(
                doc_id=hashlib.md5(f"wiki:{self.lang}:{title}".encode()).hexdigest(, usedforsecurity=False)[:16],
                titolo=title[:200],
                autore="Wikipedia",
