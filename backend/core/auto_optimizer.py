...
class MetricsCollector:
    ...
    def __init__(self, db_path: Path):
        ...
        self._provider_stats: Dict[str, ProviderStats] = defaultdict(
            lambda: ProviderStats(provider="unknown")
        )
    ...
    def _init_db(self):
        with sqlite3.connect(self.db_path) as conn:
            conn.executescript("""
                ...
                CREATE INDEX IF NOT EXISTS idx_rm_provider ON request_metrics(provider);
    ...
    def record(self, metric: RequestMetric):
        ...
        with self._lock:
            ...
            ps = self._provider_stats[metric.provider]
...
class ParameterTuner:
    ...
    def __init__(self):
        ...
        self._provider_priorities: Dict[str, float] = {}
        self._lock = threading.RLock()
    ...
    def get_optimal_params(self, domain: str, provider: str) -> Dict:
        ...
        with self._lock:
            return {
                ...
                "priority":    self._provider_priorities.get(provider, 5.0),
            }
    ...
    def get_all_params(self) -> Dict:
        with self._lock:
            return {
                ...
                "provider_priorities": dict(self._provider_priorities),
            }
...
class AutoOptimizerEngine:
    ...
    def get_health(self) -> SystemHealth:
        ...
        provider_stats = self._collector.get_provider_stats()
        recent = self._collector.get_recent(100)

        if not recent:
            return SystemHealth(
                overall_score=0.0, quality_avg=0.0, latency_avg_ms=0.0,
                cache_hit_rate=0.0, error_rate=0.0, active_providers=0,
                degraded_providers=[], status="initializing",
            )
        ...
        for prov, ps in provider_stats.items():
            if ps.total_calls >= 5:
                if ps.health_score < 0.4:
                    ...
                    self._tuner.tune_provider_priority(prov, ps.health_score)
    ...
    def get_full_report(self) -> Dict:
        ...
        return {
            ...
            "health": {
                ...
                "active_providers": health.active_providers,
                "degraded":        health.degraded_providers,
            },
            "providers": {
                prov: {
    ...
    def _run_auto_tune(self):
        ...
        for prov, ps in provider_stats.items():
            if ps.total_calls >= 10:
                self._tuner.tune_provider_priority(prov, ps.health_score)
                trend = self._analyzer.analyze_provider_trend(recent, prov)
