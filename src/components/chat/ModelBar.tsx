...
export default function ModelBar() {
  ...
  const isOllamaAvailable = (modelId: string): boolean => {
    ...
    return ollamaModels.some(
      (m) => m === modelId || m.startsWith(modelId.split(':')[0] ?? modelId),
    );
  };
    ...
    >
      ...
      >
        ...
        <span style={{ marginLeft: 'auto', color: 'var(--vio-text-dim)', fontSize: '10px' }}>
          {currentMode === 'local' ? (
            <>
              <Cpu size={10} style={{ display: 'inline', verticalAlign: 'middle' }} />{' '}
              {currentOllamaModel}
            </>
          ) : (
            <>
              <Globe size={10} style={{ display: 'inline', verticalAlign: 'middle' }} />{' '}
              {currentProvider}
            </>
          )}
        </span>
